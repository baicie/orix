...
**相关文档**：[CLI 透传、隐式 run 与 postinstall](./cli-run-passthrough-lifecycle.md)（与 [CLI & Config](./cli-config.md) 互补：参数透传、隐式 `run`、项目/依赖 `postinstall` 现状与实施顺序）。
...
orix dev                              # 隐式 run，等同 orix run dev
...
5. 将**脚本名之后**的参数追加到主脚本命令，不追加到 pre/post；无需 `--` 分隔（与 pnpm 一致）。若用户仍传入字面量 `--`，执行前剥掉一层以兼容 npm。
6. 子进程退出码原样透传；被信号终止时返回明确错误。

未匹配内置 CLI 子命令时，第一个 positional 视为脚本名（隐式 `run`），详见 [CLI 透传、隐式 run 与 postinstall](./cli-run-passthrough-lifecycle.md#2-隐式-runoidev--oi-run-dev)。
...
> **实现状态（2026-05）**：根项目 lifecycle 失败会使 `install` 失败；依赖包 lifecycle 在 link 后按 `allow-scripts` 与拓扑顺序执行。详见 [CLI 透传、隐式 run 与 postinstall](./cli-run-passthrough-lifecycle.md)。
...
| 脚本名后 `-flag` 透传（无需 `--`） | integration |
| 字面量 `--` 前缀兼容剥除 | integration |
| 隐式 `orix dev` 等同 `orix run dev` | CLI integration |
| 项目 `postinstall` 失败使 install 失败 | core integration |
| 依赖 `postinstall`（`allow-scripts`） | core integration |
