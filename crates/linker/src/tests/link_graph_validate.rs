...
#[test]
fn layout_is_invalid_when_direct_dependency_bin_shims_are_missing() -> anyhow::Result<()> {
    let temp = tempfile::tempdir()?;
    let store = Store::open(temp.path().join("store"))?;
    import_package_with_manifest(
        &store,
    ...
    fs::remove_dir_all(temp.path().join("node_modules").join(".bin"))?;

    assert!(!linker.validate_layout(&direct_deps)?.is_ok());
    Ok(())
}
